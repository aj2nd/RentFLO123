...
const FACES = [
  { id: 'front', label: 'PORTFOLIO', icon: '◈', stat: '₹2,40,000', sub: 'Total Advanced', color: T, rows: [['Active Properties', '12'], ['Tenants', '12 / 12'], ['Vacancy', 'Zero']] },
  { id: 'right', label: 'COLLECTIONS', icon: '◉', stat: '₹85,000', sub: 'Collected · May', color: '#fff', rows: [['Collection Rate', '99.2%'], ['On-Time', '100%'], ['Pending', '₹0']] },
  { id: 'back', label: 'PROPERTIES', icon: '◧', stat: '12', sub: 'Active Units', color: T, rows: [['Koramangala', '4 units'], ['Indiranagar', '3 units'], ['HSR Layout', '5 units']] },
  { id: 'left', label: 'TENANTS', icon: '◌', stat: '12 / 12', sub: 'Settled This Month', color: '#fff', rows: [['Ravi K.', '₹32,000 ✓'], ['Meera S.', '₹18,500 ✓'], ['Arjun T.', '₹27,000 ✓']] },
  { id: 'top', label: 'ANALYTICS', icon: '◈', stat: '+18.4%', sub: 'YoY Growth', color: T, rows: [['Last FY', '₹2,02,720'], ['This FY', '₹2,40,000'], ['Delta', '+₹37,280']] },
  { id: 'bottom', label: 'QUICK PAY', icon: '◎', stat: 'UPI', sub: 'Instant Settlement', color: T, rows: [['Razorpay', 'ACTIVE'], ['NEFT', 'ACTIVE'], ['Avg. Time', '< 2 min']] },
];
...
export function GlassDashboard() {
  ...
  return (
    <div style={{ minHeight: '100vh', background: '#000', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', fontFamily: 'Inter,sans-serif', overflow: 'hidden', position: 'relative' }}>
      ...
      <div style={{ position: 'absolute', width: 500, height: 500, borderRadius: '50%', background: `radial-gradient(circle, rgba(${TRGB},0.08), transparent 70%)`, filter: 'blur(40px)', pointerEvents: 'none', zIndex: 0 }} />
      ...
      <div style={{ position: 'relative', zIndex: 5, perspective: '900px', perspectiveOrigin: '50% 50%' }}>
        <div style={{ width: FACE_SIZE, height: FACE_SIZE, transformStyle: 'preserve-3d' as const, transform: `rotateX(${rx}deg) rotateY(${ry}deg)` }}>
          {FACES.map((face) => (
            <div
              key={face.id}
            ...
            >
              <div style={{ position: 'absolute', top: 0, left: 0, right: 0, height: 2, background: `linear-gradient(90deg, transparent, ${face.color}, transparent)`, opacity: activeFace === face.id ? 1 : 0.3, transition: 'opacity 0.4s' }} />
              <div style={{ position: 'absolute', inset: 10, border: `1px solid rgba(255,255,255,0.04)`, borderRadius: 6, pointerEvents: 'none' }} />
              <div style={{ position: 'absolute', inset: 18, border: `1px solid rgba(255,255,255,0.02)`, borderRadius: 4, pointerEvents: 'none' }} />
      ...
      <div style={{ position: 'relative', zIndex: 10, display: 'flex', gap: 8, marginTop: 44, flexWrap: 'wrap', justifyContent: 'center', animation: 'fadeIn 0.6s 0.2s ease-out both' }}>
        {FACES.map(f => (
          <button key={f.id} onClick={() => handleFace(f.id)} style={{
            padding: '6px 16px', fontSize: 8, letterSpacing: '0.18em', fontWeight: 700,
            borderRadius: 100,
