...
function useMouseSpring(k = 0.10, c = 0.78) {
  ...
  const onMouseMove = (e: React.MouseEvent) => {
    ...
    targetRef.current = { x: (e.clientX / W - 0.5) * 2, y: (e.clientY / H - 0.5) * 2 };
  };
...
function FloatCard({ children, style = {}, glow = false }: { children: React.ReactNode; style?: React.CSSProperties; glow?: boolean }) {
  return (
    <div style={{
      ...
      {glow && <div style={{ position: 'absolute', top: 0, left: 0, right: 0, height: 2, background: `linear-gradient(90deg, transparent, ${T}, transparent)`, borderRadius: '20px 20px 0 0' }} />}
      {children}
    </div>
  );
}
...
export function HeroLanding() {
  ...
  return (
    <div
      ...
      style={{ minHeight: '100vh', background: '#000', display: 'flex', alignItems: 'center', justifyContent: 'center', fontFamily: 'Inter,sans-serif', overflow: 'hidden', position: 'relative' }}
    >
      <style>{`
      ...
      {/* 3D scene */}
      <div style={{ position: 'relative', zIndex: 5, perspective: '1000px', perspectiveOrigin: '50% 50%', width: '100%', height: '100vh' }}>
        <div style={{ position: 'absolute', inset: 0, transformStyle: 'preserve-3d' as const, transform: `rotateX(${rotX}deg) rotateY(${rotY}deg)`, transition: 'none' }}>

          {/* Layer -200px: Grid background */}
          <Layer z={-200} style={{ inset: '-100px', pointerEvents: 'none' }}>
            <div style={{
              position: 'absolute', inset: 0,
              backgroundImage: `linear-gradient(rgba(${TRGB},0.04) 1px, transparent 1px), linear-gradient(90deg, rgba(${TRGB},0.04) 1px, transparent 1px)`,
          ...
          {/* Layer 0px: Main hero card — narrowed width */}
          <Layer z={0} style={{ top: '50%', left: '50%', transform: 'translate(-50%, -50%)' }}>
            <FloatCard style={{ width: 380 }} glow>
              <div style={{ fontSize: 8, letterSpacing: '0.28em', color: `rgba(${TRGB},0.6)`, fontWeight: 700, marginBottom: 16 }}>RENTFLO OWNER DASHBOARD</div>

              <div style={{ fontFamily: '"Playfair Display",Georgia,serif', fontSize: 46, fontWeight: 700, color: '#fff', letterSpacing: '-0.03em', lineHeight: 1.05, marginBottom: 8 }}>
                May 2026<br />
                <span style={{ color: T, textShadow: `0 0 30px rgba(${TRGB},0.4)` }}>Portfolio</span>
              </div>

              <div style={{ height: 1, background: `linear-gradient(90deg, rgba(${TRGB},0.3), transparent)`, margin: '14px 0' }} />

              {/* Grid cells — rounded */}
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8, marginBottom: 16 }}>
                {[['Total Advanced', '₹2,40,000'], ['Collected', '₹85,000'], ['Properties', '12 Active'], ['Collection', '99.2%']].map(([l, v]) => (
                  <div key={l} style={{ background: 'rgba(255,255,255,0.025)', border: '1px solid rgba(255,255,255,0.05)', borderRadius: 10, padding: '10px 12px' }}>
          ...
          {/* Layer +80px: Floating title */}
          <Layer z={80} style={{ top: 'calc(50% - 265px)', left: '50%', transform: 'translateX(-50%)' }}>
            <div style={{ textAlign: 'center', opacity: mounted ? 1 : 0, transition: 'opacity 0.5s 0.1s', whiteSpace: 'nowrap' }}>
              ...
              <div style={{ fontSize: 28, fontWeight: 200, color: '#fff', letterSpacing: '-0.03em' }}>Move your mouse</div>
            </div>
          </Layer>

          {/* Layer +140px: Action buttons */}
          <Layer z={140} style={{ bottom: 'calc(50% - 265px)', left: '50%', transform: 'translateX(-50%)' }}>
            <div style={{ opacity: mounted ? 1 : 0, transition: 'opacity 0.5s 0.5s', display: 'flex', gap: 10 }}>
              <button style={{
                padding: '13px 28px', background: T, color: '#000', border: 'none',
                borderRadius: 10,
              ...
              <button style={{
                padding: '13px 20px', background: 'rgba(255,255,255,0.04)', color: 'rgba(255,255,255,0.45)',
                border: '1px solid rgba(255,255,255,0.1)', borderRadius: 10,
                fontSize: 10, fontWeight: 500, letterSpacing: '0.12em', cursor: 'pointer',
                fontFamily: 'Inter,sans-serif',
              }}>
                LOG IN →
              </button>
          ...
          {/* Layer +200px: Status pill */}
          <Layer z={200} style={{ top: 'calc(50% - 320px)', left: '50%', transform: 'translateX(-50%)' }}>
            <div style={{
              background: `rgba(${TRGB},0.08)`, border: `1px solid rgba(${TRGB},0.25)`,
              borderRadius: 100,
              padding: '7px 18px', display: 'flex', alignItems: 'center', gap: 8, whiteSpace: 'nowrap',
