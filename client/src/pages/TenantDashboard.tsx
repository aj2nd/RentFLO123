...
export default function TenantDashboard() {
  ...
  const currentMonthBounds = useMemo(() => {
    ...
    return {
      firstDay: localIsoDate(new Date(today.getFullYear(), today.getMonth(), 1)),
      lastDay: localIsoDate(new Date(today.getFullYear(), today.getMonth() + 1, 0)),
      today: localIsoDate(today),
    };
  }, []);
  const [monthlyRentInput, setMonthlyRentInput] = useState("");
  const [dueDate, setDueDate] = useState(() => currentMonthBounds.today);
  ...
  const enteredMonthlyRent = Number(monthlyRentInput.replace(/[^0-9]/g, ""));
  const totalDue = enteredMonthlyRent > 0 ? enteredMonthlyRent : property?.monthlyRent ?? 48500;
  ...
  const amountCardValues = [
    `₹${remaining.toLocaleString("en-IN")}`,
    `₹${Math.round(remaining / 2).toLocaleString("en-IN")}`,
    `₹${Math.min(10000, remaining).toLocaleString("en-IN")}`,
    otherAmount ? `₹${Math.min(Math.max(Number(otherAmount) || 0, 0), remaining).toLocaleString("en-IN")}` : "Other",
  ];
  const dueDateLabel = useMemo(() => new Intl.DateTimeFormat("en-IN", { day: "numeric", month: "short", year: "numeric" }).format(new Date(`${dueDate}T12:00:00`)), [dueDate]);
  ...
  return <main className="dashboard-tenant bg-[#061427] text-white">
    <div className="mx-auto w-full max-w-[640px] px-0 sm:px-5 sm:py-5">
      <section className="relative aspect-[853/1844] w-full overflow-hidden sm:rounded-[30px] sm:shadow-[0_28px_80px_rgba(0,0,0,0.55)]" aria-label={`RentFLO tenant payment dashboard for ${tenantName}`}>
        ...
        <label className="absolute left-[15.5%] top-[61.6%] z-30 flex h-[3.8%] w-[25%] items-center gap-1.5 rounded-lg bg-[#132333]/90 px-1.5 text-[clamp(12px,3.1vw,20px)] font-semibold text-white backdrop-blur-sm" aria-label="Total monthly rent in Indian rupees">
          <span aria-hidden>₹</span>
          <input data-testid="input-monthly-rent" inputMode="numeric" type="text" pattern="[0-9]*" value={monthlyRentInput ? Number(monthlyRentInput).toLocaleString("en-IN") : ""} onChange={(event) => setMonthlyRentInput(event.target.value.replace(/[^0-9]/g, ""))} placeholder="— — — — —" className="min-w-0 flex-1 bg-transparent text-inherit outline-none placeholder:text-white/85" aria-label="Enter total monthly rent in Indian rupees" />
        </label>
        <label className="absolute left-[66%] top-[61.5%] z-30 flex h-[3.8%] w-[24%] items-center rounded-lg bg-[#132333]/90 px-1 text-[clamp(10px,2.5vw,16px)] font-medium text-violet-300 backdrop-blur-sm" aria-label="Select due date in the current month">
        ...
        {amountCardValues.map((value, index) => <span key={index} data-testid={`text-amount-option-${index}`} className={`pointer-events-none absolute top-[70.15%] z-30 w-[19.8%] text-center text-[clamp(8px,2.1vw,13px)] font-semibold text-white/90 ${["left-[8.7%]", "left-[29.8%]", "left-[50.9%]", "left-[71.9%]"][index]}`} aria-hidden>{value}</span>)}
        ...
        <ConnectionZone label="Choose Apple Pay" className="left-[8.7%] top-[79.1%] h-[5.4%] w-[14.8%]"><button type="button" aria-pressed={paymentMethod === "apple"} className="h-full w-full rounded-xl focus-visible:ring-2 focus-visible:ring-violet-300" aria-label="Choose Apple Pay" onClick={() => setPaymentMethod("apple")} /></ConnectionZone>
        <ConnectionZone label="Choose UPI" className="left-[26%] top-[79.1%] h-[5.4%] w-[14.8%]"><button type="button" aria-pressed={paymentMethod === "upi"} className="h-full w-full rounded-xl focus-visible:ring-2 focus-visible:ring-violet-300" aria-label="Choose UPI" onClick={() => setPaymentMethod("upi")} /></ConnectionZone>
        <ConnectionZone label="Choose PhonePe" className="left-[42.2%] top-[79.1%] h-[5.4%] w-[14.8%]"><button type="button" aria-pressed={paymentMethod === "phonepe"} className="h-full w-full rounded-xl focus-visible:ring-2 focus-visible:ring-violet-300" aria-label="Choose PhonePe" onClick={() => setPaymentMethod("phonepe")} /></ConnectionZone>
        <ConnectionZone label="Choose Google Pay" className="left-[58.5%] top-[79.1%] h-[5.4%] w-[14.8%]"><button type="button" aria-pressed={paymentMethod === "gpay"} className="h-full w-full rounded-xl focus-visible:ring-2 focus-visible:ring-violet-300" aria-label="Choose Google Pay" onClick={() => setPaymentMethod("gpay")} /></ConnectionZone>
        <ConnectionZone label="Choose Paytm" className="left-[75%] top-[79.1%] h-[5.4%] w-[14.8%]"><button type="button" aria-pressed={paymentMethod === "paytm"} className="h-full w-full rounded-xl focus-visible:ring-2 focus-visible:ring-violet-300" aria-label="Choose Paytm" onClick={() => setPaymentMethod("paytm")} /></ConnectionZone>
        {(["apple", "upi", "phonepe", "gpay", "paytm"] as PaymentMethod[]).map((method, index) => paymentMethod === method && <span key={method} data-testid="payment-selector" className={`pointer-events-none absolute z-30 top-[79.1%] h-[5.4%] w-[14.8%] rounded-xl bg-violet-400/[0.16] shadow-[inset_0_0_18px_rgba(196,181,253,0.48),0_0_18px_rgba(139,92,246,0.62)] ${["left-[8.7%]", "left-[26%]", "left-[42.2%]", "left-[58.5%]", "left-[75%]"][index]}`} aria-hidden><span className="absolute right-[8%] top-[8%] flex h-3 w-3 items-center justify-center rounded-full border border-white/80 bg-violet-600 text-[9px] font-black leading-none text-white shadow-[0_0_10px_rgba(196,181,253,1)]">✓</span><span className="absolute bottom-[8%] left-[20%] right-[20%] h-[3px] rounded-full bg-violet-100 shadow-[0_0_12px_rgba(196,181,253,1)]" /></span>)}
        <ConnectionZone label="Continue to secure rent payment" className="left-[7.5%] top-[84%] h-[4.8%] w-[69.6%]"><PayRentButton amount={selectedAmount} dueDate={dueDateLabel} vpa={RENTFLO_VPA} ledgerId={unpaidLedger?.id} presentation="image-overlay" buttonLabel="Continue to secure rent payment" ariaLabel={`Continue to pay ₹${selectedAmount.toLocaleString("en-IN")} due ${dueDateLabel}`} buttonClassName="focus-visible:ring-4" /></ConnectionZone>
        <ConnectionZone label="Home navigation" className="left-[7%] top-[91.7%] h-[8.1%] w-[13%]"><Link href="/tenant" className="block h-full w-full rounded-2xl focus-visible:ring-2 focus-visible:ring-violet-300" aria-label="Home" /></ConnectionZone>
        <ConnectionZone label="Ledger navigation" className="left-[22%] top-[91.7%] h-[8.1%] w-[13%]"><Link href="/ledger" className="block h-full w-full rounded-2xl focus-visible:ring-2 focus-visible:ring-violet-300" aria-label="Open ledger" /></ConnectionZone>
        <ConnectionZone label="Repairs navigation" className="left-[37%] top-[91.7%] h-[8.1%] w-[13%]"><Link href="/maintenance" className="block h-full w-full rounded-2xl focus-visible:ring-2 focus-visible:ring-violet-300" aria-label="Open repairs" /></ConnectionZone>
        <ConnectionZone label="Messages navigation" className="left-[52%] top-[91.7%] h-[8.1%] w-[13%]"><Link href="/messages" className="block h-full w-full rounded-2xl focus-visible:ring-2 focus-visible:ring-violet-300" aria-label="Open messages" /></ConnectionZone>
        <ConnectionZone label="Profile navigation" className="left-[67%] top-[91.7%] h-[8.1%] w-[13%]"><Link href="/profile" className="block h-full w-full rounded-2xl focus-visible:ring-2 focus-visible:ring-violet-300" aria-label="Open profile" /></ConnectionZone>
        <span className="sr-only">Monthly rent: ₹{totalDue.toLocaleString("en-IN")}. Current amount: ₹{selectedAmount.toLocaleString("en-IN")}. Due date: {dueDateLabel}. Selected payment method: {paymentMethod}.</span>
      </section>
